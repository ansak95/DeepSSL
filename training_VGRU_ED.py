...
def training_args():
    parser=argparse.ArgumentParser(description='VGRUED')
...
data_train = df[(df.ID<=9500) & (df.cycle != -1)]
data_val = df[(df.ID>9500) & (df.cycle != -1)].reset_index()
...
for rt in [60, 70, 80, 90] :
    ...
    model = VGRU_ED(input_dim=3,emb_dim = hidden_size, hidden_dim=hidden_size, output_dim=3, n_layers=nlayers, drop_prob=dropout, mean_val = trn_mean, std_val = trn_std, criterion = criterion, init_bias = bias, length_seq = args.timestep_pred, weight_kl = args.wgt)
    ...
    #create folder
    dir_path = f"VAE_{rt}"#folder_models + '/'
