...
def autoregressive_preprocess(data, sequence_length, seq_cols, timestep_pred, type_set = 'float', ratio = 1) :
    ...
    seq_gen = (list(gen_sequence_autoregressive(data[data['ID']==id], sequence_length, seq_cols, timesteps_pred=timestep_pred, h = timestep_pred, ratio = ratio)) 
                   for id in data['ID'].unique() if len(data[data['ID']==id]) >= sequence_length)
    # generate sequences and convert to numpy array
...
for rt in [60, 70, 80, 90] :
    ...
    model = GRU_Decoder(input_dim=3,emb_dim = hidden_size, hidden_dim=hidden_size, output_dim=3, n_layers=nlayers, drop_prob=dropout, mean_val = trn_mean, std_val = trn_std, criterion = criterion, init_bias = bias, length_seq = args.timestep_pred)
